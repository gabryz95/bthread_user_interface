...
public class StopProcessCommandTest {
    ...
    @Test
    public void execute() {
        stopProcessCommand.create(receiver, filename);
    ...
    public void stopProcess01() {
        stopProcessCommand.create(receiver, filename);
        assertNull(stopProcessCommand.stopProcess(filename));
    }

    @Test
    public void stopProcess02() {
        stopProcessCommand = Mockito.mock(StopProcessCommand.class);
        stopProcessCommand.stopProcess(filename);
        Mockito.verify(stopProcessCommand, Mockito.times(1)).stopProcess(filename);
    }
}